...
export default function LiveControlTower() {
  ...
  const handleIntakeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setFormData({ customerName: '', customerPhone: '', garmentType: 'Suit', fabricSelection: '', price: '', dueDate: '' });
      }
    } catch (err) {
  ...
  const advanceOrder = async (order: OrderType) => {
    ...
    // Simulate payment trigger on final step locally
    if (nextStatus === 'DISPATCHED') {
      alert(`💬 STAGE 7: DISPATCHED\nInvoice generated & simulated M-Pesa STK Push initiated for KES ${order.price} to ${order.customerPhone || 'Customer'}`);
    }
              ...
              <form onSubmit={handleIntakeSubmit} className="space-y-4 text-slate-900">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <input
                    type="text"
                    required
                    placeholder="Fabric selection"
                    value={formData.fabricSelection}
                    onChange={(e) => setFormData({ ...formData, fabricSelection: e.target.value })}
                    className="p-2.5 rounded bg-slate-800 border border-slate-700 text-slate-100 text-sm focus:border-emerald-500 focus:outline-none"
                  />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <input
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold text-xs tracking-wider rounded uppercase transition-colors"
                >
                  {loading ? 'Registering...' : 'Confirm Intake Booking'}
                </button>
              </form>
            </div>
            ...
            <div className="xl:col-span-3 overflow-x-auto pb-4">
              <div className="flex space-x-4 min-w-[1200px]">
                {STAGES.map((stage, idx) => {
                  ...
                  return (
                    <div key={stage} className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 w-72 flex flex-col h-[75vh]">
                        ...
                        {stageOrders.map((order) => (
                          ...
                          >
                            ...
                            <div className="mt-3 pt-2.5 border-t border-slate-800 flex justify-between items-center">
                              <span className="text-xs font-bold text-slate-100">KES {order.price}</span>
                              {stage !== 'DISPATCHED' && (
                                <button
